...
export default function MatchResult({
  ...
  return (
    <div className="relative h-full overflow-y-auto text-center">
      {CONFETTI.map((c, i) => (
        <span
          key={`confetti-${i}`}
      ...
      {/* min-h-full en vez de h-full: en pantallas bajas (ej. 375x667) el
          contenido puede superar el alto disponible, y como el padre ya
          tiene overflow-y-auto, esto permite hacer scroll hasta los
          botones en vez de recortarlos. */}
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 rounded-lg bg-surface-2 px-4 py-2 text-sm text-gray-100 shadow-lg">
          {toast}
        </div>
      )}
    </div>
